...
# Get UUID of block device - only use the first UUID found and ensure it's clean
ROOT_DEVICE_UUID=$(findmnt -no UUID -T / | tr -d '\n' | xargs)
if [ -z "$ROOT_DEVICE_UUID" ]; then
    # Fallback to lsblk if findmnt didn't work
    ROOT_DEVICE_UUID=$(lsblk -no UUID /dev/"$BLOCK_DEVICE" | head -n1 | tr -d '\n' | xargs)
fi
...
if command -v grub-btrfs &> /dev/null || pacman -Q grub-btrfs &> /dev/null; then
    log_info "✓ grub-btrfs is installed."
    
    # Check for grub-btrfsd service in newer versions
    if [ -f "/usr/lib/systemd/system/grub-btrfsd.service" ]; then
        log_info "Found grub-btrfsd.service (newer version format)"
        if ! systemctl is-active --quiet grub-btrfsd.service; then
            log_warning "grub-btrfsd.service is not active"
            
            # Check if inotify-tools is installed
            if ! pacman -Q inotify-tools &>/dev/null; then
                log_warning "inotify-tools not found, installing..."
                pacman -Sy --noconfirm inotify-tools
            fi
            
            log_info "Enabling and starting grub-btrfsd.service..."
            systemctl enable --now grub-btrfsd.service
            log_info "✓ Enabled grub-btrfsd.service"
        else
            log_info "✓ grub-btrfsd.service is already active"
        fi
    else
        # Check if the monitoring service is active (older versions)
        ...
        if [ -n "$GRUB_BTRFS_MONITOR_UNIT" ]; then
            ...
            log_warning "You have an older version of grub-btrfs. Consider upgrading to a newer version."
            log_info "Installing latest grub-btrfs from repositories..."
    ...
    log_info "Installing grub-btrfs from repositories..."
    if pacman -Sy --noconfirm grub-btrfs; then
        log_info "✓ grub-btrfs installed successfully."
        
        # Install inotify-tools for the daemon
        pacman -Sy --noconfirm inotify-tools
        ...
        log_error "✗ Failed to install grub-btrfs."
