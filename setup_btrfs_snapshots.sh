...
if command -v grub-btrfs &> /dev/null; then
    ...
    if sudo -u "${ORIGINAL_USER}" git clone https://aur.archlinux.org/grub-btrfs.git "${TEMP_AUR_DIR}"; then
        ...
        if sudo -u "${ORIGINAL_USER}" makepkg -s --noconfirm; then
            ...
            log_error "Failed to build grub-btrfs as user ${ORIGINAL_USER}. Please check logs or try installing it manually from AUR."
            cd ..
            rm -rf "${TEMP_AUR_DIR}"
            exit 1
        fi
        cd .. # Back to original directory before removing TEMP_AUR_DIR
        rm -rf "${TEMP_AUR_DIR}"
    else
        log_error "Failed to clone grub-btrfs AUR repository as user ${ORIGINAL_USER}."
        rm -rf "${TEMP_AUR_DIR}" # Clean up if clone failed
        exit 1
    fi
fi
...
EOF_GRUB_BTRFS
    ...
    elif ! grep "GRUB_BTRFS_SNAPSHOT_DIR=\"${SNAPSHOT_SUBVOLUME_PATH}\"" "${GRUB_BTRFS_CONFIG_FILE}"; then
        log_info "Updating GRUB_BTRFS_SNAPSHOT_DIR in ${GRUB_BTRFS_CONFIG_FILE}."
        sed -i "s|^GRUB_BTRFS_SNAPSHOT_DIR=.*|GRUB_BTRFS_SNAPSHOT_DIR=\"${SNAPSHOT_SUBVOLUME_PATH}\"|" "${GRUB_BTRFS_CONFIG_FILE}"
    fi
fi

# Systemd unit for grub-btrfs monitoring
GRUB_BTRFS_MONITOR_UNIT=""
if systemctl list-unit-files | grep -q "grub-btrfsd.path"; then
    GRUB_BTRFS_MONITOR_UNIT="grub-btrfsd.path"
elif systemctl list-unit-files | grep -q "grub-btrfs.path"; then
...
elif systemctl list-unit-files | grep -q "grub-btrfs.service"; then
...
if command -v timeshift &> /dev/null; then
    ...
    # Disable custom script's services if they were somehow enabled
    ...
    if systemctl list-unit-files | grep -q "^$(basename "${SNAPSHOT_CLEANUP_TIMER_PATH}")"; then
        ...
        systemctl stop "$(basename "${SNAPSHOT_CLEANUP_SERVICE_PATH}")" &>/dev/null # Stop associated service
    fi
else
    ...
    SERVICES_TO_ENABLE+=(
        "$(basename "${SNAPSHOT_CREATION_SERVICE_PATH}")"
        "$(basename "${SNAPSHOT_CLEANUP_TIMER_PATH}")"
    )
fi

# Always try to enable grub-btrfs monitoring unit
if [ -n "$GRUB_BTRFS_MONITOR_UNIT" ]; then
    SERVICES_TO_ENABLE+=("$GRUB_BTRFS_MONITOR_UNIT")
else
    log_warning "Skipping enablement of grub-btrfs monitoring unit as it was not found."
fi

log_info "Enabling and starting relevant systemd units..."
for unit_to_manage in "${SERVICES_TO_ENABLE[@]}"; do
    if systemctl list-unit-files | grep -q "^${unit_to_manage}"; then
        if systemctl is-enabled "${unit_to_manage}" &> /dev/null; then
            log_info "${unit_to_manage} is already enabled."
        else
            if systemctl enable "${unit_to_manage}"; then
                log_info "${unit_to_manage} enabled successfully."
            else
                log_error "Failed to enable ${unit_to_manage}."
            fi
        fi
        # Start timers and path units, or specific services like grub-btrfsd.service
        if [[ "${unit_to_manage}" == *".timer" ]] || [[ "${unit_to_manage}" == *".path" ]] || \
           [[ "${unit_to_manage}" == "grub-btrfsd.service" ]] || [[ "${unit_to_manage}" == "grub-btrfs.service" ]]; then
             if systemctl is-active "${unit_to_manage}" &> /dev/null; then
                log_info "${unit_to_manage} is already active."
             else
                if systemctl start "${unit_to_manage}"; then
                    log_info "${unit_to_manage} started successfully."
                else
                    log_error "Failed to start ${unit_to_manage}."
                fi
             fi
        fi
    else
        log_warning "Unit ${unit_to_manage} not found during enable/start phase. Skipping."
    fi
done
...
if command -v timeshift &> /dev/null; then
    ...
    if [ -n "$GRUB_BTRFS_MONITOR_UNIT" ]; then
        ...
        log_info "   If not, you might need to enable and start it: 'sudo systemctl enable --now ${GRUB_BTRFS_MONITOR_UNIT}'"
    else
